...
2. `plan`：
   ...
   - 分层批次（batch）生成
3. `execute`（MVP）：
   - `shell/http/sql` 任务执行（`http/sql` 为 MVP）
   - 重试、超时
...
1. 事件总线与可观测性：
   - 已落地可订阅 eventbus（含内存采集器），仍缺少跨进程/外部 sink 集成
2. 报告层：
   - JSON 报告已覆盖 `run/plan/validate`，包含任务耗时、状态码、批次统计、重试分布、失败聚合
   - 仍缺少更细粒度指标（如 P95、任务类型维度聚合）
3. 插件体系完善：
   - 已有统一执行注册表与内置 `shell/http/sql` 注册，并提供自定义注册入口
   - 已补生命周期协议草案（`on_load/on_init/execute/on_shutdown`）
   - 仍待实现动态加载与隔离策略
4. 并发策略深化：
   - 已有批次并发，尚未支持更细粒度执行控制和取消传播
5. 错误模型升级：
   - 已引入结构化错误码模型，并在配置、调度、执行链路接入
   - 剩余工作主要是补充更细粒度错误码和上下文字段

### 下一阶段建议（按优先级）

1. 插件体系工程化下一步：补外部插件加载与能力声明（版本/兼容性）。
2. 报告层增强：增加分位数耗时、任务类型维度聚合、失败根因归类。
3. 事件系统扩展：接入日志/指标 sink，支持订阅持久化与导出。
